...
library("corrplot")
library("factoextra")
res.pca <- PCA(data_percentage_d, quali.sup=19:20, graph = TRUE,scale.unit = TRUE)#run PCA
...
cont1<-var1 %>% 
  ...
  mutate(
    ...
    label_hjust = ifelse(PCA1 < 3, -0.1, 1.1)
  ) %>%
  ggplot(., aes(x=reorder(ID, -PCA1), y=PCA1, fill=Group))+
  geom_bar(stat='identity', color="black", alpha = 0.5)+
  geom_text(
    aes(label = ID, hjust = label_hjust),size=3)+
...
#extract legend
leg1 <- get_legend(cont1)
# Convert to a ggplot and print
leg1<-as_ggplot(leg1)
leg1

cont1<-cont1+theme(legend.position = "none")
cont1
...
cont2<-var1 %>% 
  ...
  mutate(
    ...
    label_hjust = ifelse(PCA2 > 10, 1.1,ifelse(PCA2 > 0,-0.1,ifelse(PCA2 > -15,-1,-0.5)))
  ) %>%
...
cont4<-var5 %>% 
  ...
  mutate(
    ID = factor(ID, levels = ID[order(Dim.2, decreasing = TRUE)]),
    label_hjust = c(1.1,-0.1,-0.1,2.1,1.1,-1,1.1)
  ) %>%
  ggplot(., aes(x=reorder(ID, -PCA2), y=PCA2, fill=Group))+
  geom_bar(stat='identity', color="black", alpha = 0.5)+
  geom_text(
...
sam2 <-fviz_pca_ind(res.pca1, 
  ...
  theme(plot.title = element_blank(),
        panel.border = element_rect(colour = "black", fill=NA, size=1))+
  labs(x = "PC1 (30.3%)",y="PC2 (23.5%)")+ labs(title = "Recovery") +
  theme(
    plot.title = element_textbox(
      hjust = 0.5, margin = margin(t = 5, b = 5)
    )
...
all
ggsave(plot=all,"Fig. S6.png", width = 12, height = 6)
